...
Custom sentences are only loaded from `<config>/custom_sentences/<language>/`,
never from an integration package. The templates therefore ship inside the
integration and are copied into the config directory by the
`ha_reminders.install_sentences` service, which also reloads the conversation
agent so they take effect immediately.
"""
...
SENTENCES_FILENAME = "reminders.yaml"
DEFAULT_LANGUAGE = "en"
...
async def async_install_sentences(
    ...
    """Copy the packaged reminder sentences into the config directory.
    ...
    if not source.is_file():
        raise HomeAssistantError(
            f"No packaged reminder sentences for language '{language}'"
        )
    ...
    if changed:
        _LOGGER.info(
            "Installed reminder sentences at %s%s",
            target,
            f" (previous file backed up to {backup})" if backup else "",
        )
    else:
        _LOGGER.info("Reminder sentences already up to date at %s", target)
